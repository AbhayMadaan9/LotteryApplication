...
//In the starting of contract we inform that manager has the control of contract by giving the address of contract.
    constructor()  
    {
        manager = msg.sender;  //Every smart contract invocation has a caller address. Each EVM (Ethereum Virtual Machine that executes the code) knows which account carries out each action. In Solidity, you can access the calling account by referencing msg.sender So when you call a function of solidity contract, your contract already gets the information of your account, so your account is the msg.sender
    }
    ...
    function random() public view returns(uint){
        return uint(keccak256(abi.encodePacked(block.timestamp, participants.length))); //this hash function is used to generate random from participants it is converted to uint. It is not usually used
    }
    ...
    function allParticipants() view public returns (address payable[] memory) {
        return  participants;
    }
}